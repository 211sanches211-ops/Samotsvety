...
function spawnMech(){
  ...
  for(let i=0;i<nf;i++){const cp=freeCell();if(cp)FIRE[key2(cp[0],cp[1])]=3;}
...
render=function(t){_render(t);
  ...
  for(const ch of CHAINS){const e=chainEnds(ch.id);if(e.length<2)continue;
    ...
    ctx.restore();}};
/* --- ЧТО НОВОГО: поверх всего, свои стили --- */
(function whatsnew(){
  if(store.get('sv-seen-ver','')===CUR_VER)return;
  const ov=document.createElement('div');ov.id='wnOv';
  ov.style.cssText='position:fixed;inset:0;z-index:70;display:flex;align-items:center;justify-content:center;padding:20px;background:rgba(3,10,16,.82);backdrop-filter:blur(6px);opacity:0;transition:opacity .4s;pointer-events:none;';
  ov.innerHTML='<div class="card" style="max-width:min(94vw,430px)">'+
   '<h2 class="card-title" style="font-size:clamp(20px,5vw,28px)">🆕 Что нового</h2>'+
   '<ul class="rules">'+
   '<li>◆ <b>🔥 Огонь</b>: туши совпадениями рядом, иначе взорвётся и расползётся</li>'+
   '<li>◆ <b>🔗 Цепи</b>: рви двумя совпадениями концов или молотом</li>'+
  ...
  setTimeout(()=>{ov.style.pointerEvents='auto';ov.style.opacity='1';},3000);
  ov.querySelector('#btnWn').addEventListener('click',()=>{
    store.set('sv-seen-ver',CUR_VER);ov.style.opacity='0';ov.style.pointerEvents='none';
